...
export function createBase2(
  ...
  const isGpt = typeof model === 'string' && model.toLowerCase().includes('gpt')
  ...
  return {
    ...
    systemPrompt: `You are Buffy, a strategic assistant that orchestrates complex coding tasks through specialized sub-agents. You are the AI agent behind the product, Icecode, a CLI tool where users can chat with you to code with AI.
...
- **Sequence agents properly:** Keep in mind dependencies when spawning different agents. Don't spawn agents in parallel that depend on each other.
  ${buildArray(
        ...
        isFree &&
        'Do not spawn the thinker-gpt agent, unless the user asks. Not everyone has connected their ChatGPT subscription to Icecode to allow for it.',
...
For other questions, you can direct them to icecode.com, or especially icecode.com/docs for detailed information about the product.
...
function buildImplementationStepPrompt({
  ...
  return buildArray(
    ...
    !noAskUser &&
    (isGpt
      ? `At the end of your turn, you must suggest around 3 next steps the user might want to take (even if the user just asks a question). Write these follow-up suggestions in plain Markdown text at the very end of your response under a "### Suggested Followups" section. Do NOT use the suggest_followups tool.`
      : `Always provide a normal user-facing response first.
Afterwards, you must use the suggest_followups tool
to suggest around 3 next steps.
Never use suggest_followups as the sole response.`),
  ).join('\n')
}
