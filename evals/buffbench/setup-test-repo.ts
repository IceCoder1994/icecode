...
export async function setupTestRepo(
  ...
  try {
    ...
    const githubToken = process.env.ICECODE_GITHUB_TOKEN
    ...
    if (
      ...
      console.error(
        '1. Verify ICECODE_GITHUB_TOKEN environment variable is set',
      )
      console.error(
      ...
      if (token) {
        ...
        console.error('ICECODE_GITHUB_TOKEN environment variable is not set')
      }
    }
