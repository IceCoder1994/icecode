...
export const Chat = ({
  ...
  // Custom model sessions have no backend server to check connection or auth against.
  // Override authStatus and isConnected so the status bar doesn't show a permanent
  // "connecting..." or "retrying..." message.
  const isCustomModelSession =
    freebuffSession?.status === 'active' && isCustomModelId(freebuffSession.model)
  const effectiveIsConnected = isCustomModelSession ? true : isConnected
  const effectiveAuthStatus = isCustomModelSession ? 'ok' as const : authStatus
  ...
  const inputBoxTitle = useMemo(() => {
    ...
    if (modelDisplayName) {
      segments.push(`🤖 ${modelDisplayName}`)
    }
  ...
  }, [modelDisplayName, queuePreviewTitle, pausedQueueText])
