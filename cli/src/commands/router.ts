...
import { buildCustomModelPlanPrompt, buildInterviewPrompt, buildPlanPrompt, buildReviewPrompt } from './prompt-builders'
...
export async function routeUserPrompt(
  ...
  if (inputMode === 'plan') {
    ...
    const planPrompt = isCustom
      ? buildCustomModelPlanPrompt(trimmed)
    ...
    sendMessage({ content: planPrompt, agentMode })
    setTimeout(() => {
      scrollToLatest()
    }, 0)
    return
