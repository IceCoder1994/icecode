...
// Base prompt for plan command when using a custom model (no ChatGPT connection needed)
export const CUSTOM_MODEL_PLAN_BASE_PROMPT =
  'Gather all the relevant context and then think deeply and create a detailed implementation plan for the following:'
...
/**
 * Build a plan prompt for custom model users — does not reference @thinker-gpt
 * since custom models should not connect to ChatGPT/OpenAI for planning.
 * @param input - The user's plan request (e.g., "add OAuth login")
 * @returns The full prompt to send to the agent
 */
export function buildCustomModelPlanPrompt(input: string): string {
  const trimmedInput = input.trim()
  if (!trimmedInput) {
    return CUSTOM_MODEL_PLAN_BASE_PROMPT
  }
  return `${CUSTOM_MODEL_PLAN_BASE_PROMPT}\n\n${trimmedInput}`
}
