...
function parseArgs(): ParsedArgs {
  ...
  // 仅保留 Freebuff：简化 CLI，无 agent 覆盖、无模式切换、无发布入口。
  program
    .name('freebuff')
  ...
  // 仅保留 Freebuff，固定使用 LITE。
  const initialMode: AgentMode | undefined = 'LITE'
...
async function main(): Promise<void> {
  ...
  const AppWithAsyncAuth = () => {
    ...
    React.useEffect(() => {
      // Bypass login: always allow access without authentication.
