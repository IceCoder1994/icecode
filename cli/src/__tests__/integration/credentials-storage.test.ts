 ...
 * - Dev: ~/.config/icecode-dev/credentials.json
 * - Prod: ~/.config/icecode/credentials.json
 *
 * Tests ensure:
...
describe('Credentials Storage Integration', () => {
  ...
  beforeEach(() => {
    ...
    tempConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'icecode-test-'))
  ...
  describe('P0: File System Operations', () => {
    ...
    test('should use icecode-test directory in test environment', async () => {
      ...
      expect(configDir).toEqual(
        path.join(os.homedir(), '.config', 'icecode-test'),
      )
    })

    test('should use icecode-dev directory in development environment', async () => {
      // Restore getConfigDir to use real implementation for this test
      mock.restore()
      ...
      const configDir = authModule.getConfigDir()
      expect(configDir).toEqual(
        path.join(os.homedir(), '.config', 'icecode-dev'),
      )
    })

    test('should use icecode directory in production environment', async () => {
      ...
      const configDir = authModule.getConfigDir()
      expect(configDir).toEqual(path.join(os.homedir(), '.config', 'icecode'))
    })
