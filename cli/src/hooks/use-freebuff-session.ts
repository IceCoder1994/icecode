 ...
 *
 *  Custom model sessions (instanceId === 'custom') are purely local and have
 *  no server-side row to release. */
function shouldReleaseSlot(current: FreebuffSessionResponse | null): boolean {
  if (!current) return false
  if (current.instanceId === 'custom') return false
  return (
...
async function releaseFreebuffSlot(
  current: FreebuffSessionResponse | null = useFreebuffSessionStore.getState().session,
): Promise<void> {
...
async function restartFreebuffSession(
  ...
  controller?.abort()
  // 返回 landing 时先本地切屏，避免等待清理请求时 UI 看起来无响应。
  if (mode === 'landing') {
    if (controller) {
      await controller.restart(mode)
    } else {
      // 控制器偶发缺席时，仍要先把界面切回模型选择页。
      const { setSession, setError } = useFreebuffSessionStore.getState()
      setSession(toLandingSession(currentSession))
      setError(null)
    }
    if (opts.resetChat) await resetChatStore()
    if (opts.releaseSlot) await releaseFreebuffSlot(currentSession)
    return
  }
  if (opts.resetChat) await resetChatStore()
  if (opts.releaseSlot) await releaseFreebuffSlot(currentSession)
 ...
 *
 * Custom models bypass the queue entirely — the session is set to 'active'
 * directly by the model picker, so this function is a no-op for them.
 */
export function joinFreebuffQueue(model: string): Promise<void> {
  if (!IS_FREEBUFF) return Promise.resolve()
  // Custom models skip the server queue entirely.
  if (isCustomModelId(model)) {
    ...
    // Session already set to 'active' by the picker; nothing more to do.
...
export function useFreebuffSession(): UseFreebuffSessionResult {
  ...
  useEffect(() => {
    ...
    // Custom model selected — bypass the waiting room entirely and go
    // straight to 'active'. The agent runtime will use the user's own
    // API endpoint instead of the Codebuff server's model queue.
    const initialModel = getSelectedFreebuffModel()
    if (isCustomModelId(initialModel)) {
      setSession({
        status: 'active',
        instanceId: 'custom',
        model: initialModel,
    ...
    if (!token) {
      logger.warn(
        {},
        '[freebuff-session] No auth token; landing on model picker (no backend)',
      )
      // No auth token means no backend server to talk to. Go straight to
      // landing so the user can pick a custom model or configure one.
    ...
    const tick = async () => {
      ...
      try {
        ...
        // On the very first tick (no previous status), gracefully fall back
        // to landing state instead of showing an error. This lets users
        // configure and use custom models without a backend server running.
