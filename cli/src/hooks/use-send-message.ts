    ...
    async ({ content, agentMode, postUserMessage, attachments }) => {
      ...
      if (!client) {
        ...
        setMessages((prev) => [
          ...prev,
          createErrorChatMessage(
            '⚠️ Unable to connect to Freebuff. Please check your authentication and try again.',
          ),
        ])
        await yieldToEventLoop()
        setTimeout(() => scrollToLatest(), 0)
      ...
      try {
        ...
        const selectedModelId = getSelectedFreebuffModel()
        let customModelEndpoint: import('@codebuff/sdk').CustomModelEndpointConfig | undefined
        if (selectedModelId && isCustomModelId(selectedModelId)) {
          const customModel = getCustomModelById(selectedModelId)
          if (customModel) {
            customModelEndpoint = {
