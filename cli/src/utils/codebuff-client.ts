...
export async function getCodebuffClient(): Promise<CodebuffClient | null> {
  if (!clientInstance) {
    ...
    // Custom models don't need a Codebuff API key — the SDK routes LLM calls
    // directly to the user's configured endpoint. Log a debug message and
    // continue with a placeholder so the client can be instantiated.
    if (!apiKey) {
      logger.debug(
        {},
        'No Codebuff API key found. The client will use a placeholder key — ' +
        'this is expected when using custom model endpoints.',
      )
    }
