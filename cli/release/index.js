...
const releaseRepository = 'IceCoder1994/icecode'
const releaseTagPrefix = 'icecode-v'
...
function createConfig(packageName) {
  ...
  const configDir = path.join(homeDir, '.config', packageName)
  const binaryName =
...
function trackUpdateFailed(errorMessage, version, context = {}) {
  try {
    ...
    const payload = JSON.stringify({
      ...
      event: 'cli.update_icecode_failed',
      properties: {
...
function getReleaseAssetUrl(tagOrLatest, fileName) {
  return `https://github.com/${releaseRepository}/releases/download/${tagOrLatest}/${fileName}`
}

function getLatestReleaseAssetUrl(fileName) {
  return `https://github.com/${releaseRepository}/releases/latest/download/${fileName}`
}

function getVersionFromResolvedUrl(resolvedUrl) {
  if (!resolvedUrl) return null
  const match = resolvedUrl.match(/\/download\/icecode-v([^/]+)\//)
...
async function getLatestReleaseVersion() {
  ...
  try {
    const res = await httpGet(getLatestReleaseAssetUrl(fileName))
    const version = getVersionFromResolvedUrl(res.resolvedUrl)
    res.resume()
...
async function getLatestAvailableVersion() {
  const [npmVersion, releaseVersion] = await Promise.all([
    getLatestVersion(),
    getLatestReleaseVersion(),
  ])

  if (!npmVersion) return releaseVersion
  if (!releaseVersion) return npmVersion

  return compareVersions(npmVersion, releaseVersion) >= 0
    ? npmVersion
    : releaseVersion
}
...
async function downloadBinary(version) {
  ...
  const downloadUrl = getReleaseAssetUrl(`${releaseTagPrefix}${version}`, fileName)
  ...
  let res = await httpGet(downloadUrl)
  let resolvedVersion = version

  if (res.statusCode !== 200) {
    if (res.statusCode === 404) {
      const fallbackUrl = getLatestReleaseAssetUrl(fileName)
      term.writeLine(
        `Exact release asset missing for ${version}, falling back to latest GitHub Release...`,
      )
      res.resume()
      res = await httpGet(fallbackUrl)
      const fallbackVersion = getVersionFromResolvedUrl(res.resolvedUrl)
      if (fallbackVersion) {
  ...
  const exactResolvedVersion = getVersionFromResolvedUrl(res.resolvedUrl)
  if (exactResolvedVersion) {
  ...
  term.clearLine()
  console.log(`Download complete! Starting icecode${resolvedVersion ? ` (${resolvedVersion})` : ''}...`)
}
...
async function ensureBinaryExists() {
  ...
  const version = await getLatestAvailableVersion()
  if (!version) {
...
function printCrashDiagnostics(code, signal) {
  ...
  console.error(`  https://github.com/${releaseRepository}/issues`)
  console.error('')
}
