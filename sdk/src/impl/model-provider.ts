 ...
 * - Custom Model: Direct requests to user-configured OpenAI-compatible endpoint
...
/**
 * Configuration for a user-configured custom model endpoint.
 */
export interface CustomModelEndpointConfig {
...
export interface ModelRequestParams {
  ...
  costMode?: string
  /** Custom model endpoint config — if provided, bypasses Codebuff backend entirely */
...
export interface ModelResult {
  ...
  /** Whether this model is a user-configured custom endpoint model */
 ...
 *
 * Priority order:
 * 1. Custom model endpoint (if provided) — bypasses everything
 * 2. ChatGPT OAuth direct (if available)
 * 3. Codebuff backend (default)
 *
...
export async function getModelForRequest(params: ModelRequestParams): Promise<ModelResult> {
  const { apiKey, model, skipChatGptOAuth, costMode, customModelEndpoint } = params

  // Priority 1: Custom model endpoint — talk directly to user's configured API
  if (customModelEndpoint) {
    return {
      model: createCustomEndpointModel(customModelEndpoint, model),
      isChatGptOAuth: false,
      isCustomModel: true,
    }
  }

  // Priority 2: ChatGPT OAuth direct
...
/**
 * Create an OpenAI-compatible model that talks directly to a user-configured endpoint.
 */
function createCustomEndpointModel(
  config: CustomModelEndpointConfig,
  ...
  return new OpenAICompatibleChatLanguageModel(config.modelId, {
    provider: 'openai',
    url: ({ path: endpoint }) => {
      // endpoint is like "/chat/completions" or "/embeddings"
    ...
    fetch: undefined, // Use global fetch
