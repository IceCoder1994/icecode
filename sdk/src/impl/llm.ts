...
export async function* promptAiSdkStream(
  ...
  const modelParams: ModelRequestParams = {
    ...
    customModelEndpoint: (params as any).customModelEndpoint,
  }
  const { model: aiSDKModel, isChatGptOAuth, isCustomModel } =
  ...
  // Skip cost tracking for ChatGPT OAuth and custom models (user is on their own subscription)
  if (!isChatGptOAuth && !isCustomModel) {
...
export async function promptAiSdk(
  ...
  const { model: aiSDKModel, isCustomModel } = await getModelForRequest(modelParams)
  ...
  // Call the cost callback if provided — skip for custom models
  if (!isCustomModel && params.onCostCalculated && costOverrideDollars) {
    await params.onCostCalculated(
      calculateUsedCredits({ costDollars: costOverrideDollars }),
    )
  }
