...
import type { CustomModelEndpointConfig } from './impl/model-provider'
...
export class CodebuffClient {
  ...
  constructor(options: CodebuffClientOptions) {
    ...
    // Use a placeholder API key when none is provided — this allows the client
    // to be used with custom model endpoints that don't require Codebuff auth.
    const effectiveApiKey = foundApiKey ?? 'local-dev-placeholder-key'
   ...
   * @param customModelEndpoint - (Optional) Custom model endpoint configuration. If provided, all LLM calls route through this endpoint instead of the Codebuff backend.
   *
